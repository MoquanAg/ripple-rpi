...
logger = GlobalLogger("RippleEC", log_prefix="ripple_").logger
...
class EC:
    ...
    def is_connected(self):
        ...
        Note:
            ...
            - Attempts to read the EC register (0x0000) with a short timeout
            - Returns False on timeout or communication error
        """
        try:
            # Try to read EC register to verify connectivity
            response = self.modbus_client.read_holding_registers(
                port=self.port,
                address=self.REGISTERS['ec'],  # 0x0000
                count=1,
                slave_addr=self.address,
                baudrate=self.baud_rate,
                timeout=0.5,
                device_name='EC_connectivity_check'
            )

            connected = not response.isError() and response.registers is not None
            if not connected:
                logger.debug(f"EC sensor {self.sensor_id} not responding")
            return connected

        except Exception as e:
            logger.debug(f"EC sensor {self.sensor_id} connectivity check failed: {e}")
            return False
